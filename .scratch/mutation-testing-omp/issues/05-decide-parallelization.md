...
2. **Custom Gradle task for result capture**: Write a Gradle task that runs mutflow tests and outputs structured JSON with fields: `pointId`, `variantIndex`, `result` (Killed/Survived/TimedOut), `killedByTest` (test name). Cross-reference with JUnit XML for test-level metadata. This provides the per-mutation data the auditor needs.
...
| 4 | Custom Gradle task outputs structured JSON (pointId, variantIndex, result, killedByTest). JUnit XML provides per-test names. Auditor cross-references to build mutation score + zombie detection data. |
